...
import { OptimizerSGD, Params, randn, tensor, Tensor } from "./api";
...
export class Trainer {
  ...
  async step() {
    ...
    if (this.opt.steps % 10 === 0) {
      ...
      const x = tensor(this.lossSteps);
      const y = tensor(this.lossValues);
      plt.plot(x, y);
    }
